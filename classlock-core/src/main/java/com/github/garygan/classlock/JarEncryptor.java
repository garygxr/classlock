...
public class JarEncryptor {
    ...
    static {
        ...
        aopMap.put("spring.code", "char[] c=${passchar};"
                + "is=com.github.garygan.classlock.JarDecryptor.getInstance().decryptConfigFile(this.path,is,c);");
        ...
        aopMap.put("jfinal.code", "char[] c=${passchar};inputStream=com.github.garygan.classlock.JarDecryptor.getInstance().decryptConfigFile(fileName,inputStream,c);");
    ...
    public void addClassFinalAgent() {
        ...
        thisJarPaths.forEach(thisJar -> {
            ...
            if ("jar".endsWith(this.jarOrWar) && thisJar.endsWith(".jar")) {
                List<String> includeFiles = Arrays.asList(Const.CLASSLOCK_FILES);
            ...
            //本项目开发环境中未打包
                ...
                files.forEach(file -> {
                    ...
                    if (file.isDirectory()) {
                        targetFile.mkdirs();
                    } else if (StrUtils.containsArray(file.getAbsolutePath(), Const.CLASSLOCK_FILES)) {
